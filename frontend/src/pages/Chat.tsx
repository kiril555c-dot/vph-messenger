...
const Chat: React.FC = () => {
  ...
  // Обработка сокетов — с проверками на null
  useEffect(() => {
    if (!socket) return;
  ...
  // Если user не загружен — показываем загрузку, чтобы не было пустоты
  if (!user) {
    return <div className="flex h-screen items-center justify-center text-white text-xl">Загрузка профиля...</div>;
  }
