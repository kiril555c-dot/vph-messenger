...
const Chat: React.FC = () => {
  ...
  // МГНОВЕННОЕ ОТКРЫТИЕ ЧАТА (ИСПРАВЛЕНО)
  const startChat = async (targetUser: any) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // 1. Сначала подтягиваем актуальный список чатов с сервера для синхронизации
        await fetchChats(token || '');

        // 2. Делаем новый чат активным
        setActiveChat(chat);
        ...
        // 4. Подключаем сокет к новому каналу
        socket?.emit('join_chat', chat.id);
        
        // 5. Очищаем локальные сообщения, чтобы окно было готово к новому чату
        setMessages([]); 

        console.log("Чат инициализирован и открыт без перезагрузки");
      }
    } catch (e) {
